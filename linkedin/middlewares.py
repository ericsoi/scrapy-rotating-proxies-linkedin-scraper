...
with open("/home/eric/http_proxies.txt", "r") as file:
...
class ProxyMiddleware:
    ...
    @classmethod
    def from_crawler(cls, crawler):
        proxies = ["http://" + ip for ip in lines]
        return cls(proxies)
    ...
    def process_response(self, request, response, spider):
        if response.status in [403, 429, 503]:
            # If the response indicates an error status, switch to the next proxy
            self.change_proxy(request)
            return request
        return response

    def process_exception(self, request, exception, spider):
        # If an exception occurs during the request, switch to the next proxy
        self.change_proxy(request)
        return request
    ...
    def change_proxy(self, request):
        if 'proxy' in request.meta:
            ...
            if self.proxies:
                ...
                print(f"Switched proxy to: {new_proxy}")
